...
public:
    ...
    Graph_SearchBFS( graph_type& graph,
                    int source,
                    Node  target ):
    m_Graph(graph),
    m_iSource(source),
...
bool Graph_SearchBFS<graph_type>::Search() {
    ...
    while (!queue.empty()) {
        ...
        //如果找到目标，返回
        Node tar = m_Graph.GetNode(Next->To());
        
        if (tar.GetA()==m_ntarget.GetA()&&tar.GetB()==m_ntarget.GetB()&&tar.GetC()==m_ntarget.GetC()) {
            m_ntarget.setIndex(Next->To());
            return true;
        }
        
        m_Graph.addHanoidNode(Next->To());
        m_Visitied.resize(m_Graph.NumNodes(),unvisited);
        m_Route.resize(m_Graph.NumNodes(),no_parent_assigned);
        //将这条边指向的节点的所有相关联的边入栈
        //只要关联的边指向的节点没有被访问过
        //获得图中边的迭代器
...
std::list<int > Graph_SearchBFS<graph_type>::GetPathToTarget()const {
    ...
    if (!m_bFound) return path;
    
    int nd = m_ntarget.Index();
    path.push_back(nd);
    while (nd != m_iSource) {
        nd = m_Route[nd];
        path.push_back(nd);
